...
class MainActivity : AppCompatActivity() {
    ...
    private fun setupNavigationButtons() {
        ...
        for ((id, activityClass) in buttonActivityPairs) {
            // Перевіряю, чи кнопка існує, перш ніж встановити слухач
        ...
        findViewById<ImageButton>(R.id.likeButton3)?.setOnClickListener {
            ...
            // Запускаю FavoritesActivity через Intent
    ...
    private fun observePlaylists() {
        ...
        // Спостерігаю за LiveData 'playlistItems' з ViewModel
        playlistsViewModel.playlistItems.observe(this, Observer { items ->
            ...
            // Передаю змішаний список адаптеру
            playlistAdapter.submitList(items)

            // Показую/ховаю відповідні елементи
    ...
    private fun showCreatePlaylistDialog() {
        ...
        builder.setPositiveButton("Створити") { dialog, _ ->
            ...
            if (playlistName.isNotEmpty()) {
                // Використовую rawPlaylists для перевірки ліміту
    ...
    private fun showRenamePlaylistDialog(playlist: Playlist) {
        ...
        // Створюю поле вводу і встановлюю поточну назву
        val inputLayout = TextInputLayout(this).apply {
            setPadding(50, 30, 50, 30)
            hint = "Нова назва"
        }
        val input = EditText(this).apply {
            setText(playlist.name) // Встановлюю поточну назву в поле вводу
            setSelection(playlist.name.length) // Ставлю курсор в кінець тексту
        }
        inputLayout.addView(input)
        builder.setView(inputLayout)
        ...
        builder.setPositiveButton("Зберегти") { dialog, _ ->
            ...
            // Перевіряю, чи назва не порожня і чи вона змінилася
            if (newName.isNotEmpty() && newName != playlist.name) {
                // Викликаю метод ViewModel для оновлення назви
            ...
            dialog.dismiss() // Закриваю діалог
        }
        // Кнопка "Скасувати"
    ...
    private fun showAddSoundDialog(targetPlaylist: Playlist) {
        ...
        // Тепер отримую LiveData зі звуками через ViewModel (ПУБЛІЧНЕ ПОЛЕ)
        val allSoundsLiveData = playlistsViewModel.allSounds // Використовуємо публічне поле

        allSoundsLiveData.observe(this) { allSounds -> // ВИПРАВЛЕНО: Лямбда-синтаксис
            // Видаляю спостерігач одразу, щоб діалог не з'являвся повторно
            if (allSounds != null) {
            ...
            AlertDialog.Builder(this@MainActivity)
                ...
                .setPositiveButton("Додати") { _, _ ->
                    ...
                    if (soundsToAdd.isNotEmpty()) {
                        ...
                        playlistsViewModel.addSoundsToPlaylist(soundsToAdd, targetPlaylist.id) // Викликаю метод ViewModel
