...
class PlaylistRepository(private val playlistDao: PlaylistDao) {
    ...
    suspend fun getSoundsForPlaylist(soundIds: List<String>): List<Sound> {
        if (soundIds.isEmpty()) {
            return emptyList() // Повертаю порожній список, якщо ID немає
        }
    ...
    suspend fun addSoundsToPlaylist(soundIdsToAdd: List<String>, playlistId: Int) {
        if (soundIdsToAdd.isEmpty()) return // Якщо нічого додавати - вихід

        Log.d("RepoAddSounds", "Repo: Adding sounds $soundIdsToAdd to playlist $playlistId")
        try {
            val playlist = playlistDao.getPlaylistById(playlistId) // Читаю плейлист ОДИН РАЗ
            if (playlist != null) {
                val currentSoundIds = playlist.soundIds.toMutableList()
                var changed = false
                // Додаю всі нові ID, яких ще немає
                ...
                // Якщо список реально змінився, оновлюю БД ОДИН РАЗ
                if (changed) {
                    val updatedPlaylist = playlist.copy(soundIds = currentSoundIds.toList()) // Копіюю з оновленим списком
                    Log.d("RepoAddSounds", "Repo: Updating playlist ${playlist.id} in DB. New sounds: ${updatedPlaylist.soundIds}")
                    playlistDao.updatePlaylist(updatedPlaylist) // Записую ОДИН РАЗ
                } else {
        ...
        try { // Додамо try-catch для безпеки
            ...
            if (playlist != null) {
                ...
                if (removed) {
                    ...
                    playlistDao.updatePlaylist(updatedPlaylist) // Оновлюю в базі
